...
const UserListPagination = (props) => {
  ...
  return (
    <div className="flex flex-col bg-[rgba(255,255,255,0.21)] rounded-2xl p-2">
      ...
      <div className="join self-center rounded mt-2 flex gap-2">
        <button className="join-item btn" onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1}>
          «
        </button>
        
        <div className="join-item dropdown dropdown-bottom dropdown-center p-0">
            <div tabIndex={0} role="button" className="btn rounded-none">Page {currentPage}</div>
            <ul tabIndex={0} className="dropdown-content menu backdrop-blur-sm bg-base-300/50 rounded-box z-1 w-52 p-2 mt-1 shadow-lg">
        ...
        <button className="join-item btn" onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages}>
          »
        </button>
      </div>
    </div>
